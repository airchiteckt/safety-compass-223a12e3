...
import { scadenze, dipendenti } from "@/data/mock";
...
export default function FormazionePage() {
  ...
  const corsi = scadenze.filter((s) => s.tipo === "formazione");
  const filtered = corsi.filter(
    (s) =>
      s.elemento.toLowerCase().includes(search.toLowerCase()) ||
      s.azienda.toLowerCase().includes(search.toLowerCase()) ||
      s.riferimento.toLowerCase().includes(search.toLowerCase())
  );

  const scaduti = corsi.filter((s) => s.status === "scaduto").length;
  const inScadenza = corsi.filter((s) => s.status === "in_scadenza").length;

  return (
    <AppLayout>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-foreground">Formazione</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {corsi.length} corsi totali · {scaduti} scaduti · {inScadenza} in scadenza
          </p>
        </div>
      ...
      <div className="relative mb-6 max-w-sm">
        ...
        <input
          type="text"
          placeholder="Cerca corso, azienda, dipendente..."
          value={search}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                <th className="px-4 py-3 text-left font-medium text-muted-foreground">Corso</th>
