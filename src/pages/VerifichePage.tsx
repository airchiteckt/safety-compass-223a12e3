...
export default function VerifichePage() {
  ...
  const verifiche = scadenze.filter((s) => s.tipo === "verifica_impianto");
  const filtered = verifiche.filter(
    (s) =>
      s.elemento.toLowerCase().includes(search.toLowerCase()) ||
      s.azienda.toLowerCase().includes(search.toLowerCase())
  );

  const scadute = verifiche.filter((s) => s.status === "scaduto").length;
  const inScadenza = verifiche.filter((s) => s.status === "in_scadenza").length;

  return (
    <AppLayout>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-foreground">Verifiche Impianti</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {verifiche.length} verifiche totali · {scadute} scadute · {inScadenza} in scadenza
          </p>
        </div>
      ...
      <div className="relative mb-6 max-w-sm">
        ...
        <input
          type="text"
          placeholder="Cerca verifica, azienda..."
          value={search}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                <th className="px-4 py-3 text-left font-medium text-muted-foreground">Impianto</th>
