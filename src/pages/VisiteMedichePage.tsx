...
export default function VisiteMedichePage() {
  ...
  const visite = scadenze.filter((s) => s.tipo === "visita_medica");
  const filtered = visite.filter(
    (s) =>
  ...
  const scadute = visite.filter((s) => s.status === "scaduto").length;
  const inScadenza = visite.filter((s) => s.status === "in_scadenza").length;

  return (
    <AppLayout>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-foreground">Visite Mediche</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {visite.length} visite totali · {scadute} scadute · {inScadenza} in scadenza
          </p>
        </div>
      ...
      <div className="relative mb-6 max-w-sm">
        ...
        <input
          type="text"
          placeholder="Cerca visita, azienda, dipendente..."
          value={search}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                <th className="px-4 py-3 text-left font-medium text-muted-foreground">Tipo Visita</th>
